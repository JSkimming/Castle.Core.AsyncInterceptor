...
public class AsyncDeterminationInterceptor : IInterceptor
{
    ...
    private static readonly MethodInfo HandleAsyncEnumerableMethodInfo =
        typeof(AsyncDeterminationInterceptor)
            .GetMethod(nameof(HandleAsyncEnumerableWithResult), BindingFlags.Static | BindingFlags.NonPublic)!;
#endif
    ...
    public virtual void Intercept(IInvocation invocation)
    {
        ...
        switch (methodType)
        {
            ...
            case MethodType.AsyncFunction:
                ...
                return;
#if NET5_0_OR_GREATER
            case MethodType.AsyncEnumerable:
                GetAsyncEnumerableHandler(invocation.Method.ReturnType).Invoke(invocation, AsyncInterceptor);
                return;
#endif
    ...
    private static MethodType GetMethodType(Type returnType)
    {
        ...
        if (typeof(IAsyncEnumerable<>).IsAssignableFrom(genericTypeDef ??= returnType.GetGenericTypeDefinition()))
            return MethodType.AsyncEnumerable;
#else
    ...
    /// Gets the <see cref="GenericAsyncHandler"/> for the method invocation <paramref name="returnType"/>.
    /// </summary>
    private static GenericAsyncHandler GetAsyncEnumerableHandler(Type returnType)
    {
        GenericAsyncHandler handler = GenericAsyncHandlers.GetOrAdd(returnType, CreateAsyncEnumerableHandler);
        return handler;
    }
    ...
    /// Creates the generic delegate for the <paramref name="returnType"/> method invocation.
    /// </summary>
    private static GenericAsyncHandler CreateAsyncEnumerableHandler(Type returnType)
    {
        Type taskReturnType = returnType.GetGenericArguments()[0];
        MethodInfo method = HandleAsyncEnumerableMethodInfo.MakeGenericMethod(taskReturnType);
        return (GenericAsyncHandler)method.CreateDelegate(typeof(GenericAsyncHandler));
    }
    ...
    /// <typeparam name="TResult">The type of the <see cref="Task{T}"/> <see cref="Task{T}.Result"/> of the method
    /// <paramref name="invocation"/>.</typeparam>
    private static void HandleAsyncEnumerableWithResult<TResult>(IInvocation invocation, IAsyncInterceptor asyncInterceptor)
    {
        asyncInterceptor.InterceptAsynchronousEnumerable<TResult>(invocation);
    }
